...
(defonce gcode-model-path-cache (atom {}))

(defn- gcode-cache-key [gcode-data]
...
(defn- cached-model-path [gcode-data projection]
  (let [cache-key [(gcode-cache-key gcode-data) projection]]
    (if-let [path (get @gcode-model-path-cache cache-key)]
      path
      (let [path (path-for-segments projection
                                    (:bounds gcode-data)
                                    (filter :extruding? (:segments gcode-data)))]
        (swap! gcode-model-path-cache
               (fn [cache]
...
(defn- gcode-viewbox [bounds projection]
...
(def ^:private static-gcode-model-svg
  (r/create-class
...
(defn- gcode-svg
  ([label projection bounds segments segment-index current-segment]
   (gcode-svg label projection bounds segments segment-index current-segment {}))
  ([label projection bounds segments segment-index current-segment
    {:keys [model-path model-key current-only? cursor-radius show-travel? show-upcoming?]
     :or {show-travel? true
          show-upcoming? true
          cursor-radius 0.9}}]
   (let [viewbox (gcode-viewbox bounds projection)
     ...
     [:div {:class (str "gcode-stage-panel gcode-stage-" (name projection))}
      ...
      (when-not (str/blank? model-path)
        ...
        [static-gcode-model-svg viewbox model-path])
...
(def ^:private gcode-highlight-segment-limit 64)
...
(defn- gcode-stage [gcode-data correlation]
  (let [segments (vec (:segments gcode-data))
        ...
        highlight-segments (recent-segments segments segment-index gcode-highlight-segment-limit)
        model-key (gcode-cache-key gcode-data)]
    (if (and bounds current-segment)
      [:div {:class "gcode-stage-grid"}
       [gcode-svg "top / current layer" :top bounds current-layer-segments segment-index current-segment]
       [gcode-svg "side / full shape" :side bounds highlight-segments segment-index current-segment
        {:model-path (cached-model-path gcode-data :side)
         :model-key [model-key :side]
         :cursor-radius 1.9
         :show-travel? false
         :show-upcoming? false}]
       [gcode-svg "isometric / full shape" :iso bounds highlight-segments segment-index current-segment
...
(defn- gcode-context [gcode-data correlation]
  (let [segments (vec (:segments gcode-data))
...
(defn- replay-toolpath-panel [gcode-data replay-data packet-range current-packet-msg]
  [:section {:class "replay-part-panel"}
   [:div {:class "replay-panel-head"}
    ...
    [:span "current layer"]]
   (if gcode-data
     (let [correlation (replay-correlation gcode-data replay-data packet-range current-packet-msg)]
       [:<>
        [gcode-stage gcode-data correlation]
       [gcode-context gcode-data correlation]])
